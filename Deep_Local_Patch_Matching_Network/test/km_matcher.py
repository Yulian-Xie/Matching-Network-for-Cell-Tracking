...
class KMMatcher:
    ...
    def solve(self, verbose = False):
        ...
        for x in range(self.n):
            if verbose:
                print('match {} to {}, weight {:.8f}'.format(x+1, self.xy[x]+1, self.weights[x, self.xy[x]]))
                score[str(x)] = [x+1, self.xy[x]+1, self.weights[x, self.xy[x]]]  #真实匹配的细胞序号，j+1，k+1，匹配权重
                weight.append(self.weights[x, self.xy[x]])
            sum += self.weights[x, self.xy[x]]
        self.best = sum
        if verbose:
            print('ans: {:.6f}'.format(sum))
        return score, weight
